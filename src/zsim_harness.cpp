...
int main(int argc, char *argv[]) {
    CLI::App app{"zsim: a simulator"};

    app.add_flag_callback("-v", 
            []() {printf("%s\n", ZSIM_BUILDVERSION);
                    exit(0);
    });

    std::string cfg_file;
    app.add_option("config_filename", cfg_file, "A cfg file")->required();
    CLI11_PARSE(app, argc, argv);
    info("You selected config file '%s'", cfg_file.c_str());
    ...
    const char* configFile = realpath(cfg_file.c_str(), nullptr);
